...
def _get_python_info():
    ...
    for cmd in [["py", "-3", "--version"], ["python3", "--version"], ["python", "--version"]]:
        try:
            ...
            sys_exe = subprocess.check_output(path_cmd, stderr=subprocess.STDOUT, timeout=5).decode().strip()
    ...
    try:
        out = subprocess.check_output([core_exe, "--version"], stderr=subprocess.STDOUT, timeout=5).decode().strip()
    ...
    try:
        out = subprocess.check_output([tray_exe, "--version"], stderr=subprocess.STDOUT, timeout=5).decode().strip()
...
def build_update(ctx):
    ...
    # ══════════════════════════════════════════════════════════════════════════
    install_card = _card(container, "\U0001f4e5  Install & Setup")
    ...
    if core_ok:
        try:
            ...
            core_ver = "?"
        badge_text, badge_color = f"\u2705  Hecos Core installed  \u2014  v{core_ver}  at  {_ROOT}", "#4ade80"
    else:
        badge_text, badge_color = f"\u26a0  Core not found at: {_ROOT}  \u2014  Download it to get started.", ACCENT

    ctk.CTkLabel(install_card, text=badge_text, font=ctk.CTkFont(size=11), text_color=badge_color, justify="left", wraplength=640).pack(anchor="w", padx=16, pady=(0, 10))
    ...
    dl_btn_text = "\U0001f4e5  Re-Download Core" if core_ok else "\U0001f4e5  Download Core"

    dl_btn = ctk.CTkButton(btn_row, text=dl_btn_text, fg_color=BORDER if core_ok else ACCENT, text_color=TEXT if core_ok else "#000", hover_color=ACCENT, **btn_cfg)
    dl_btn.grid(row=0, column=0, padx=(0, 6), sticky="ew")

    setup_btn = ctk.CTkButton(btn_row, text="\u2699  Run Setup Wizard", fg_color=BORDER, text_color=TEXT, hover_color=ACCENT, state="normal" if core_ok else "disabled", **btn_cfg)
    ...
    def do_download():
        ...
        def _task():
            ...
            def stat(msg): install_status.configure(text=msg, text_color=RED if "\u26a0" in msg else MUTED)
            ...
            if success:
                ...
                dl_btn.configure(state="normal", text="\U0001f4e5  Re-Download Core", fg_color=BORDER, text_color=TEXT)
    ...
    def do_setup():
        ...
        def stat(msg): install_status.configure(text=msg, text_color=RED if "\u26a0" in msg else "#4ade80")
    ...
    # ══════════════════════════════════════════════════════════════════════════
    upd_card = _card(container, "\U0001f504  Check & Install Updates")

    if not core_ok:
        ctk.CTkLabel(upd_card, text="\u26a0  Install Hecos Core first to enable updates.", font=ctk.CTkFont(size=11), text_color=MUTED).pack(anchor="w", padx=16, pady=(0, 14))
    else:
        ...
        upd_progress.set(0)
        upd_btn = ctk.CTkButton(upd_card, text="\U0001f504  Check for Updates", fg_color=ACCENT, text_color="#000", hover_color=ACCENT, height=34)
        ...
        def do_check():
            ...
            def _task():
                ...
                if err:
                    ...
                    upd_btn.configure(state="normal", text="\U0001f504  Check for Updates", command=do_check)
            ...
            def _task():
                try:
                    ...
                    for i, asset in enumerate(target_assets):
                        upd_status.configure(text=f"Downloading {asset['name']} ({i+1}/{len(target_assets)})\u2026", text_color=MUTED)
                    ...
                    upd_status.configure(text=f"\u26a0 Download failed: {e}", text_color=RED)
                    upd_progress.pack_forget()
                    upd_btn.configure(state="normal", text="\U0001f504  Check for Updates", command=do_check)
            threading.Thread(target=_task, daemon=True).start()
